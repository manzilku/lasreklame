...
$(document)['ready'](function(akrilik) {
  var erl = -1,
    land = '',
    erland = '';
  akrilik('#menu')['find']('ul')['find']('li')['each'](function() {
    for (var ramd = akrilik(this)['text'](), dhani = akrilik(this)['find']('a')['attr']('href'), ramdhani = 0, erlandme = 0; erlandme < ramd.length && (ramdhani = ramd.indexOf('_', ramdhani), -1 != ramdhani); erlandme++) {
      ramdhani++
    };
    if (level = erlandme, level > erl && (land += '<ul>', erland += '<ul>'), level < erl) {
      offset = erl - level;
      for (var erlandme = 0; erlandme < offset; erlandme++) {
        land += '</ul></li>', erland += '</ul></li>'
      }
    };
    ramd = ramd.replace(/_/gi, ''), land += '<li><a href=\'' + dhani + '\'>' + ramd + '</a>', erland += '<li><a href=\'' + dhani + '\'>';
    for (var erlandme = 0; erlandme < level; erlandme++) {
      erland += ''
    };
    erland += ramd + '</a>', erl = level
  });
  for (var ramdhani = 0; erl >= ramdhani; ramdhani++) {
    land += '</ul>', erland += '</ul>', 0 != ramdhani && (land += '</li>', erland += '</li>')
  };
  akrilik('#menu .LinkList')['html'](erland), akrilik('#menu > .LinkList > ul')['attr']('id', 'nav1'), akrilik('#menu ul > li > ul')['parent']('li')['addClass']('parent'), akrilik('#menu .widget')['attr']('style', 'display:block!important;')
});
